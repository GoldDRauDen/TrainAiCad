...
### 2.5 Approved deletion of clearly crossed-out hole groups (V4.8)

- If a visibly crossed-out entire hole-family callout AND its leader/target scope unambiguously cancel a complete family, DELETE every corresponding hole from the produced flat-pattern CAD, even when obsolete drawn circles are still visible. Record source family, nominal callout count, clearly established strike-through and leader scope, effective quantity zero, and the adjacent uncanceled families.
- The mark need not be red when the entire canceled callout and its target scope are explicit and no newer revision conflicts. A newer inconsistent revision follows Section 4.1; if chronology or scope cannot be proven, STOP and ask.
- A line through only a tolerance (e.g., H7), finish requirement, dimension fragment, or a single word is NOT proof that the entire hole family was canceled. Retain independently proved holes and ask about any unresolved machining requirement. Never silently delete on a partial or ambiguous crossing.
- Suppress canceled circles, pilot holes, POINTs and obsolete finishing warnings. Feature-count closure is applied separately to active families and to each confirmed canceled family (required CAD entity count zero). Revalidate geometry and translated composite. Do not use a reference DXF as a substitute for PDF-first cancellation scope evidence.
- Approved regression: on original PDF page 8, code 040006, the struck-through full 2-Ø8H7 family is confirmed canceled: delete BOTH holes; preserve the independently called-out 6-M8 and 4-Ø9. This specific confirmed case does not make every crossed-out H7 notation a general hole deletion.
...
Explicit full-family cancellation with a proven leader scope is governed by Section 2.5. Do not preserve obsolete callouts merely because their symbols remain on an older view; a newer conflicting revision or partial strike-through requires re-evaluation.
...
- **Feature Count Closure Check**: For every explicit quantity callout (`n-Ø`, `n-M*`, `n-slot`, `n-R`, `n-C`, repeated notch family, etc.), the generated family count MUST equal `n`. Thread-to-pilot conversion changes diameter representation, not the required count. Missing or extra members are FAIL, not WARN. First classify active versus proven canceled callouts under Section 2.5. A fully canceled family requires ZERO surviving entities, while every retained family must independently meet its stated quantity.
...
- **V4.8 cancellation-scope audit:** Assert zero entities of each fully canceled hole family in individual and composite DXFs; preserve separate active counts and remove obsolete finishing warnings. If only a tolerance/finish fragment was crossed or its target scope is unclear, retain proven holes and request clarification.
...
### B.5 V4.8 approved calibration 520709-07 (2026-09-26)

- Enforce Section 2.5 clear whole-family cancellation; distinguish full hole removal from tolerance-only crossing.
- Apply Appendix F for 039915 OD/step 25 vs erroneous 22.36, 039918/039919 printed slope 50 plus the PDF-correct M4/Ø5 mapping and complete mirror, and 040006 confirmed deletion of 2-Ø8H7 with 6-M8/4-Ø9 retained.
- V4.7 dual DXF export plus manifest and all V4.6 material/Nobi/POINT/Laser R standards remain unchanged. The V4.7 historical snapshot must never be overwritten; no corrected production DXF export is asserted by this approval.

### B.6 V4.7 user-approved DXF-first switch (2026-09-26)
...
### B.7 V4.6 approved `055958` single-slit correction (2026-09-25)
...
### B.8 V4.5 approved L09–L10 extension (2026-09-25)
...
### B.9 V4.4 approved 520924-19 release
...
### B.10 V4.3 Portability Change Log (historical only)
